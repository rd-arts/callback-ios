...
@synthesize webView;
@synthesize localRoot = _localRoot;
...
- (NSString*) pathForResource:(NSString*)resourcepath {
    ...
    NSString* directoryStr = self.localRoot;
    
    if ([directoryPartsJoined length] > 0) {
        directoryStr = [NSString stringWithFormat:@"%@/%@", self.localRoot, [directoryParts componentsJoinedByString:@"/"]];
    }
...
-(id) getCommandInstance:(NSString*)pluginName {
    ...
    if (!obj) 
    {
        ...
        if (classSettings) {
            obj = [[NSClassFromString(className) alloc] initWithViewController:self webView:self.webView settings:classSettings];
        } else {
            obj = [[NSClassFromString(className) alloc] initWithViewController:self webView:self.webView];
        }
